...
let resArray=[]
searchInput.onkeydown=(e)=>{if(e.target.value){let xhr=new XMLHttpRequest()
xhr.onreadystatechange=()=>{if(this.readyState==4&&this.status==200){debugger}}
xhr.open("GET","{{url_for('users.search')}}"+`?name=${e.target.value}`,true)
xhr.send()}}